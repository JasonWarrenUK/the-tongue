...
<div class="overflow-x-auto" style="max-height:30vh">
  <svg width={W} height={H} style="min-width:100%">
    {#each eraGraph.edges as e (e.from + "->" + e.to)}
      <line x1={cx(e.from)} y1={cy(e.from) + NH} x2={cx(e.to)} y2={cy(e.to)} stroke="var(--color-border)" stroke-width="1.5" />
    {/each}
    {#each eraGraph.nodes as node (node.key)}
      ...
      <g transform={`translate(${cx(node.key) - NW / 2}, ${cy(node.key)})`} role="button" tabindex="0"
        ...
        <text x={NW / 2} y="15" text-anchor="middle" fill={selected ? "var(--color-accent)" : leaf ? "var(--color-fg)" : "var(--color-muted)"} font-size="12" font-weight="600">{node.stage.text}</text>
        <text x={NW / 2} y="27" text-anchor="middle" fill="var(--color-muted)" font-size="9">{node.isTerminal ? (leaf ? `${branches[node.branchId].history.length} chg` : "ancestor") : node.stage.bucket}</text>
