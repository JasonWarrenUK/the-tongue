...
A seeded, deterministic language-evolution simulator: sound-change rules drift a lexicon across autonomously drifting, spreading and fracturing branches, tracked through a mutual-intelligibility matrix. Milestone 1 (Core Simulator) is complete; Milestone 2 (Depth & Legibility) is queued behind a chain of design spikes; Milestone 3 (Persistence & Sharing) is a deliberately deferred stub. Milestone 4 (Beyond Current Scope) ships nothing: it holds work a spike judged too costly for its value now, kept so the reasoning survives.
...
- [x] **1ENG.17** — Implement the three adoptions from the Zompist survey (1eng-16 spike §7), three independent slices: (1) frequency-ranked phoneme selection — gen's geometric dropoff as `pickRanked` in `rng.ts` (single-draw inverse-CDF, so inventory size can't shift downstream world-gen draws), replacing `pick` at `genSyllable`'s four call sites; (2) distance conditioning — an optional `Rule.distance` predicate scanning to the word edge (SCA²'s `…`) with `xform`'s ctx widened to carry the found segment, plus a new `umlaut` rule, making long-distance assimilation expressible for the first time; (3) metathesis — a neighbour-consuming `Seg` variant riding 1ENG.12's existing `Seg[]` output shape, plus a `metath` rule. Breaking: slice 1 changes what every seed generates (`feat(engine)!:`); slices 2 and 3 are additive (no test asserts a rule count, verified at survey time) though drift-replay goldens shift as `driftRule`'s weighted total changes. **Flagged by the 1ENG.24 spike:** its slice 2 here and 1ENG.24 slice 2 (1ENG.30) both propose widening `Rule.xform`'s ctx object — this task with a distance-found segment, 1ENG.24 with a `stress?: StressCtx` field. Independent optional fields, so compatible, but whichever lands second must extend the object rather than replace it. **Done — three deviations from the spike text, all ledgered:** `genInventory`'s consonant list was rebuilt against a fixed `CONSONANT_RANK` (it used to be draw order, not a stable rank, so `pickRanked`'s dropoff didn't mean what it claimed); `umlaut` ships as a feature-copy off the found trigger (`nasassim`'s shape) rather than the spike's literal `xform`, which doesn't compile and would have silently deleted most vowel types via the self-seg-patch trap `reduce`'s comment documents; `metath` got a new `"metathesis"` category (affinity 0.0) rather than `"assimilation"`, since metathesis is reordering, not feature-spreading. A real bug found and fixed in the same pass: a moved `/r/` could silently become `/l/` (featurally identical in this engine's consonant model) via `resolveSeg`'s neighbour-diff path — fixed by special-casing an empty patch to the neighbour's own id _(depends on 1ENG.16)_
...
- [x] **1ENG.21** — Implement 1ENG.14 stage B: word-order pressure drivers — new turn-loop step 3.75 with the morphological-collapse rigidification driver (reads 1ENG.20's paradigm state, revokes pro-drop; `ORDER_TURNS` clock) and the intense-contact alignment driver (reads 2GEO.5's `pairContact`; `ORDER_CONTACT_CUT`), `Branch.orderPressure`/`orderContactPressure` and UI warnings mirroring fracture/assimilation. The divergent fracture-birth reanalysis flip ships earlier in stage A — contract specified in the amended 1ENG.14 spike. **Done — deviations, all ledgered:** "revokes pro-drop" needed no new write (`licensesProDrop` already recomputes false at two dead cells, strictly before the three-dead collapse trigger); "one step toward the neighbour's order" is swap distance on the S/O/V permutohedron (Ferrer-i-Cancho et al., arXiv 2604.26726), so `SOV↔VSO` takes two events via `SVO`. **Two reachability bugs found and fixed via live multi-seed playtesting** — both mechanisms passed every unit test yet never fired in real autonomous play: `pairContact` never exceeded 0.5 on the pre-existing 4×3 map (below `ORDER_CONTACT_CUT` 0.6) — `genRegions` grown to 10×8 (2GEO.9 filed for map-shape diversification beyond a bigger grid); `ORDER_TURNS` and `RENEWAL_TURNS` were both 6, and step 1 ran before step 3.75 in the same turn, so renewal always revived the paradigm one tick before rigidification's threshold could check it — fixed by dropping `ORDER_TURNS` to 5. Stochastic turn-clock thresholds flagged on 2SIM.1 as a design question, not resolved here _(depends on 1ENG.19, 1ENG.20, 2GEO.5)_
...
- [x] **1ENG.24** — Design spike: stress, and the sound changes that need it — found that 1eng-25 §5.3's pre-specified whole-word stress gate (mirroring the salience/syntax gates in applyRuleToLex) CANNOT WORK: every high-volume rule targets the word's final syllable (post:bound), and whether the final syllable is stressed is a property of the branch's stress mode alone, not the word, so the gate degenerates to a per-branch constant (measured: final-syllable-stressed at 0%/100%/0%/0% across the four modes). Worse, a whole-word gate is all-or-nothing and cannot express either of the roadmap's named targets ("delete the unstressed vowel, keep the stressed one" is inherently sub-word). Amends 1eng-25 §5.3: departs from item 1 (the per-word gate), keeps item 4 verbatim (a per-branch seeded+inherited parameter reading syllabify() live), and dissolves the item-2/item-3 roll-composition contradiction rather than adjudicating it — stress now conditions the transducer per-segment via a widened xform ctx and a fail-closed Rule.stressed predicate, so syntaxMult's existing roll is untouched. Blast radius smaller than expected: zero of the 17 existing RULES change (TypeScript structural typing makes the widened ctx free for every consumer that ignores it). Re-measured the corpus post-1ENG.27: 1.82 syll/word, 52.2% polysyllabic, 20.1% >=3 syllables at turn 80 — the 95%-monosyllabic floor 1eng-25 §3 measured is gone. Also specifies a real schwa (tri-valued Phone.back: "front"|"central"|"back", not a central? boolean, which would silently corrupt to /e/ via applyXform) and the honest syllable-weight test (coda OR long vowel OR diphthong, not coda-only — moves heavy-penult rate from 1.7% to 13.6%, narrowly below the 15% "live lever" threshold, ledgered as flavour). Contract in `docs/spikes/1eng-24-stress.md`
- [x] **1ENG.29** — Implement 1ENG.24 slice 1: a real schwa — widen `Phone.back`/`Patch.back` from `boolean` to `"front"|"central"|"back"`, add a schwa phone, and fix `frontV` (phonology.ts:52) to an explicit `=== "front"` test. Schwa is excluded from genesis inventories for free (`genInventory` builds vowels from hardcoded literals, never from `PHONES`) — it only enters a branch's live inventory once a reduction rule produces it, which is the attested pattern. `feat(engine)!:` (a public type widens) but no drift-replay goldens move (no new vowel enters any genesis inventory)
- [x] **1ENG.30** — Implement 1ENG.24 slice 2: the stress substrate — `StressRule`/`StressCtx` types, `Rule.stressed` (fail-closed positional gate), `stressPosition`/`stressMap` in `syllable.ts`, `applyRuleToWord`'s widened ctx and optional `StressRule` parameter, `World.stressRule`/`Branch.stressRule` with genesis draw (tail-appended after `adj`) and fracture-birth inheritance. No new `hashRand` salt needed (`seed+47` stays free — every draw is a genesis `rng()` tail-append or fully pure). `feat(engine)!:` (required `Branch` field, ~40 test call sites) but no goldens move (no rule yet declares `stressed`)
- [x] **1ENG.31** — Implement 1ENG.24 slice 3: `reduce` (unstressed vowel reduction to the new schwa, w=2.5) and `syncope` (unstressed syllable loss, w=2, the Latin→French engine) added to `RULES`, both gated by `Rule.stressed`. The vowel floor (phonology.ts:189) is structurally unreachable from `syncope` by construction (stressPosition always exempts one nucleus) — pinned as an invariant test, not a second guard. `feat(engine)!:`, goldens move (two new rules change `driftRule`'s weighted total, every seed diverges from turn ~1)
...
- [ ] **2GEO.9** — Design spike: map-shape diversification — `genRegions` generates a fixed rectangular grid; 1ENG.21 grew it 4×3 (12 regions) to 10×8 (80) purely to fix `pairContact`'s `ORDER_CONTACT_CUT` reachability, not as a map-generation improvement in its own right. Explore varied shapes and non-rectangular layouts (irregular coastlines, non-uniform region density, procedurally varied grid dimensions per seed) so every world's geography doesn't read as the same rectangle at a different scale — the actual gap this task exists to close, surfaced but out of scope during 1ENG.21's playtest fix
- [x] **2GEO.10** — Investigation spike: family-structure reachability (`docs/spikes/2geo-10-family-structure-reachability.md`) — a 12-seed census (surfaced while planning 2STK.4) found fracture/assimilation death never fire autonomously on the 10×8 grid: genesis connectivity sits far above the bond-percolation threshold. Terrain-probability tuning alone produces a bimodal outcome (0 fractures, or unbounded branch-count runaway once impassable share rises far enough), traced to a spread/fracture feedback loop (the boxed-in-terrain-crossing fallback creates disconnection that fracture immediately resolves into a fresh boxed-in fragment, repeat) rather than a probability to find — a wider 40-seed sample confirmed even a moderate nudge leaves 87.5% of seeds at zero. Genuine reachability needs better map generation, squarely 2GEO.9's territory. Ships two safety fixes regardless, both real bugs independent of reachability: `FRACTURE_COOLDOWN` (`Branch.fractureCooldown`, rate-limits re-fracturing) and `STUCK_TURNS` (a time-based crossing floor so a genesis-walled-in branch escapes eventually instead of freezing for the whole run — an earlier, blunter `territory >= 2` gate was tried and rejected for having no escape valve). Terrain probability ships unchanged from baseline. Census harness `docs/spikes/assets/2geo-10-family-census.ts` committed for 2GEO.9/2SIM.1 to extend
...
- [ ] **2STK.4** — Implement ambitions & lock-in (2stk-1 spike §4) — `World.ambitions` via constrained tension draw, live-condition evaluation each repool, natural deadlines only, lock-in grading and freezing the record, goal-driven → sandbox phase flag, starter roster (self-referencing ambitions need focusId; roster items reading momentum/contact state activate as 2STK.3/2STK.5 land). **Planning found 7 of the original 10-item roster (Babel, Ozymandias, Twins, Last Word, Matriarch, Scatterling, Exile) need fracture or assimilation death, which 2GEO.10 found near-unreachable autonomously** — genuine reachability now depends on 2GEO.9's map-generation work, not just 2GEO.10's safety fixes. Also carry forward 2GEO.10's roster corrections (Proto- needs ≥2 descendants not ≥3; Ozymandias/Long Tongue/Babel/Scatterling condition fixes) and the self-anchoring finding (every ambition should require the focal branch's own participation, not a god's-eye condition) _(blocked — depends on 2STK.1, 2STK.2, 2GEO.10, 2GEO.9)_
...
- [ ] **2SIM.1** — Integration pacing census — re-run the census harness with every mechanic active (borrowing, collision repair, syntax gating, paradigms): combined event density per branch-turn, collision dynamics at 48 concepts under the class-gate + distance model, affix lifetime distribution with the explicit acceptance criterion that grammaticalisation cycles observably turn (else the 1ENG.15 backstop question reopens with data in hand), and a tuning pass across the constant family (`COLLISION_TURNS`, `SEVERITY_CUT`, `BORROW_RATE`, `SYNTAX_STRENGTH`, `FRAME_WALK`, `ORDER_*`). **Flagged during 1ENG.21 implementation:** a live playtest sweep found `ORDER_TURNS`/`RENEWAL_TURNS` both at 6 made rigidification structurally unreachable (a same-turn race — renewal always ran first and revived a fully collapsed paradigm before rigidification's own threshold could check it); fixed narrowly by dropping `ORDER_TURNS` to 5, but it raises a broader question this census should take up: should any sustained-N-turns threshold vary stochastically rather than being a fixed integer, and if so how the "N/M turns until X" UI convention would need to adapt _(depends on 2GEO.5, 2LEX.2, 1ENG.20, 1ENG.22)_
...
graph LR
	...
	2GEO.10["2GEO.10: Investigation spike: family-structure r…"]
	...
	1ENG.29["1ENG.29: Implement 1ENG.24 slice 1: a real schw…"]
	1ENG.30["1ENG.30: Implement 1ENG.24 slice 2: the stress…"]
	1ENG.31["1ENG.31: Implement 1ENG.24 slice 3: `reduce` (u…"]
...
- [ ] **4PHON.1** — Design spike: tonogenesis — tone arising from lost segmental contrasts (the Sinitic and Vietnamese histories are largely tonogenetic; the LCK names it as a change type worth modelling). **Deliberately deferred beyond the current roadmap, not merely unscheduled:** tone is suprasegmental, so it needs a new axis on `Phone`, a tone-bearing-unit notion (specified by 1ENG.25, implemented by 1ENG.28), a reckoning with all 21 rules, and a decision about how `intelligibility.ts`'s Levenshtein metric should weigh a tone difference against a segment difference — that last one is a genuine design question, since tone differences are perceptually salient in ways a naive edit distance would understate. Recorded so the 1ENG.16 survey's judgement (cost, not merit) stays on the record rather than being rediscovered. Revisit no earlier than 1ENG.24, and only once M1-M3 have shipped. **Updated by the 1ENG.24 spike:** the tone-bearing-unit prerequisite is discharged (1ENG.28's `syllabify`), and 1ENG.24 slice 1 (1ENG.29) does the `Phone`-axis widening this task would otherwise have had to do itself for tone, while 1ENG.24 slice 2 (1ENG.30)'s `Rule.stressed` establishes the per-segment conditioning channel a tone rule will also need. `dependsOn` stays on 1ENG.24 (the spike, now done) rather than 1ENG.31, since this task's own spike is still unscheduled and should read 1ENG.24's contract, not wait on its implementation
