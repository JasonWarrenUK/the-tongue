...
- [ ] **1ENG.19** — Implement the 1ENG.14 substrate and conditioning (stage A) — `CONCEPTS` 32→48 (incl. the `finish` amendment from the 1ENG.15 spike) with `CONCEPT_CLASS`, the class-pure `SEMANTIC_FIELDS` extension, new `syntax.ts` (`FRAMES`, `frameOrder`, `positionProfile`, `followerVowelShare`, `syntaxMult`, `SYNTAX_STRENGTH`), `Branch.wordOrder` seeded and inherited (no drift yet), the `applyRuleToLex` syntax gate at both drift call sites and `PhrasePanel.svelte`. Breaking: required `Branch.wordOrder`, `CONCEPTS` growth shifts goldens — contract specified in the 1ENG.14 spike; sequenced after 2LEX.2 so the semantic-fields table isn't extended against a moving target _(blocked — depends on 1ENG.14, 2LEX.2)_
- [ ] **1ENG.20** — Implement the 1ENG.15 paradigm model — new `morphology.ts` (`seedParadigm`, `tickParadigm`, `inflect`, `PATHWAY`, `AFFIX_EROSION_RATE`/`RENEWAL_TURNS`/`FUSE_TURNS`), `Branch.paradigm` (breaking: required field), the paradigm tick inside drift (step 1), genesis seeding in `world.ts`, fracture deep-copy inheritance and the PhrasePanel/paradigm-chip rendering — contract specified in the 1ENG.15 spike _(blocked — depends on 1ENG.15, 1ENG.19)_
- [ ] **1ENG.21** — Implement 1ENG.14 stage B: per-branch word-order drift — new turn-loop step 3.75 with the morphological-collapse rigidification driver (reads 1ENG.20's paradigm state; `ORDER_TURNS` pressure clock) and the intense-contact alignment driver (reads 2GEO.5's `pairContact`; `ORDER_CONTACT_CUT`), `Branch.orderPressure`/`orderContactPressure` and UI warnings mirroring fracture/assimilation — contract specified in the 1ENG.14 spike _(blocked — depends on 1ENG.19, 1ENG.20, 2GEO.5)_
...
graph LR
	...
	1ENG.21["1ENG.21: Implement 1ENG.14 stage B: per-branch…"]
	...
	2UI.2 --> M2
	3SHR.1 --> M3
	class 1ENG.16,2GEO.5,3PER.1 todo
	class 1ENG.17,1ENG.19,1ENG.20,1ENG.21,2GLY.1,2GLY.2,2GLY.3,2GLY.4,2LEX.2,2STK.1,2STK.2,2UI.1,2UI.2,3SHR.1 blocked
