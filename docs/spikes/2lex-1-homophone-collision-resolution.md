...
**Changed — `src/lib/engine/generation.ts`**: new step 1.5 per §4 (tick counters against `severePairs`, delete healed keys, repair at each pair's `pairThreshold` via `resolveCollision`, log `"X disambiguated 'moon' as 'skymoon'"` — corrected during 2LEX.2 implementation: neither `moon` nor `sun` is terrain-salient, so the index tie-break fires and `sun` keeps the short form per §3.3, not the reverse this section originally stated); fracture birth copies `collisionPressure` to children. **Also corrected during implementation:** the autonomous-repair cap is `MAX_REPAIRS_PER_TURN = 3` ripest pairs per branch per turn, not one — a live-engine census (not the passive pre-mechanic census this spike ran) found up to 14 concurrent severe pairs on a single branch-turn, so a strict one-per-turn cap left a permanent backlog rather than ever clearing the chronic tail. Verified via a 30-seed/150-turn sweep with the mechanic active: zero severe pairs remain stuck past their own `pairThreshold` at horizon, while tolerated (sub-cut/cross-class) homophones persist untouched.
