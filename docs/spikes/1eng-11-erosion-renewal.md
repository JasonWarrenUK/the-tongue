...
- **Compensatory lengthening (→ 1ENG.13):** deferred, but its prerequisite (long-vowel phones) is delivered by this contract, so it becomes a small rule-only task — a deletion rule whose `xform` deletes a coda *and* lengthens the preceding vowel (a 2→1-with-trace pattern the widened transducer already supports via `Seg[]` at the preceding position).
