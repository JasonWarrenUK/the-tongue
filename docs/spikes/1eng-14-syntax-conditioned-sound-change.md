...
> **Revised (1ENG.21 implementation).** Stage B shipped with three deviations from this section's text and two bugs found and fixed via live multi-seed, multi-hundred-turn playtesting — every synthetic/unit test passed throughout, since they all hold state artificially collapsed turn-over-turn, which is exactly what masked both bugs.
>
> **"Revokes `proDrop`" needed no new write.** `licensesProDrop` already recomputes to `false` at TWO dead agreement cells, strictly before `agreementCollapsed`'s three-dead rigidification trigger — so by the time rigidification can fire, `proDrop` is already `false` via the existing per-turn recompute in `generation.ts` step 1. Pinned as an ordering test (`proDrop is already false at the moment rigidification fires`) rather than duplicated as a second write, which would have created two sources of truth for one field.
>
> **"One step toward the neighbour's order" needed a real metric.** With three admitted values (SOV/SVO/VSO) there is no inherent ordering between them. Shipped: swap distance on the S/O/V permutohedron — the minimum number of adjacent-constituent transpositions to turn one order into another (Ferrer-i-Cancho et al., *Swap distance minimization shapes the order of subject, object and verb in languages of the world*, arXiv 2604.26726 — the established metric for this exact question). Computed via BFS over all six logical orders (`syntax.ts`'s `BasicOrder`), not just the three `WordOrder` admits, so a future task widening `WordOrder` needs no change to the metric. Among the three admitted values the swap graph is the line SOV–SVO–VSO, so `SOV↔VSO` takes two events via `SVO` — independently corroborated by the diachronic finding that SOV word-order drift passes through SVO before (if ever) reaching VSO.
>
> **Bug: rigidification was structurally unreachable.** `ORDER_TURNS` and `RENEWAL_TURNS` (morphology.ts) were both `6`, and turn-loop step 1 (`tickParadigm`) runs before step 3.75 in the same turn. On the 6th consecutive collapsed turn, `tickParadigm`'s renewal clock reached `RENEWAL_TURNS` and revived all three dead cells to `periphrastic` — in the SAME turn rigidification's own `orderPressure` counter would have reached `ORDER_TURNS` and fired. Renewal always ran first and won the race, every time, so rigidification could never see a paradigm that was still collapsed at its own threshold. Every dedicated unit test passed because they all re-force the paradigm collapsed after each `resolveGeneration` call, bypassing the natural renewal path entirely — a live playtest sweep (many seeds, hundreds of turns, natural autonomous erosion) found zero rigidification events ever, which is what surfaced it. Fixed by dropping `ORDER_TURNS` to `5`, so rigidification's check wins the race instead. Whether either clock should vary stochastically rather than being a fixed integer is a real, separate design question, filed on `2SIM.1` for its constant-tuning pass rather than resolved here.
>
> **Bug: contact alignment was reachable in principle, unreachable on the shipped map.** `pairContact` never exceeded 0.5 across thousands of real pair-checks on the pre-existing 4×3 (12-region) map, while `ORDER_CONTACT_CUT` is `0.6` — a branch typically bordered 2+ neighbours on a map that small, capping any single neighbour's border share below the cut. `genRegions` (`geography.ts`) grows to 10×8 (80 regions); every consumer walks `Region[]`/`Edge[]`/`Adjacency` generically with no region-count assumption, audited before the change, so this was safe on its own. `2GEO.9` filed for map-shape diversification beyond just a bigger fixed grid, since that is a real, separate gap this fix surfaced but did not set out to close.
